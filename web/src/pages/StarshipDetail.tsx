...
export function StarshipDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        setLoading(true);
        const response = await fetch(`/api/starships/${id}`);
        if (!response.ok) {
        ...
        setStarship({ ...(data as Starship), id });
    ...
    <main className="mx-auto max-w-5xl px-4 py-12 text-gray-100">
      ...
      <section className="card grid gap-8 border border-yellow-500/30 bg-[#101628]/80 shadow-[0_0_35px_rgba(255,213,79,0.08)] sm:grid-cols-[320px_1fr]">
        ...
        <div className="space-y-6">
          ...
          {/* Basic Info */}
          <div>
            <h2 className="text-lg font-heading text-yellow-200 mb-3">
            ...
            <div>
              ...
              <div className="space-y-4">
                {starship.weapons.map((weapon, idx) => (
                  <div
                    key={idx}
                    className="border-l-2 border-yellow-400/30 pl-3"
                  >
          ...
          {starship.notes && (
            <div>
              ...
              <p className="text-sm text-gray-200/80 whitespace-pre-line">
                {starship.notes}
              </p>
            </div>
          )}
