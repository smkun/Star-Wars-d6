  ...
  useEffect(() => {
    async function loadVariants() {
      ...
      try {
        ...
        const all = await speciesApi.fetchSpeciesList();
        const ships: Starship[] = Array.isArray(all) ? all : [];

        const matches = ships
          .filter((d: any) => {
            if (!d.parent) return false;
            const parentClean = String(d.parent)
              .replace(/'''/g, '')
              .replace(/\[\[|\]\]/g, '')
              ...
              .trim()
              .toLowerCase();
            return parentClean === capitalizedName.toLowerCase();
          })
          .map((d: any) => ({ id: d.slug || d.id, ...d }) as Starship);
        ...
        setError(
          `Unable to load variants. ${err instanceof Error ? err.message : 'Please try again later.'}`
        );
      } finally {
        setLoading(false);
      }
      ...
      {variants.length === 0 ? (
        <div className="card border border-yellow-500/30 bg-[#101628]/80 text-center">
          <p className="text-gray-300">
            No variants found for this starship family.
          </p>
        </div>
      ) : (
        (() => {
