...
/**
 * Set parent/variant relationships for TIE fighters
 * All TIE variants should have parent: "TIE Fighter" except the base TIE Fighter itself
 *
 * This script detects the Firestore emulator via FIRESTORE_EMULATOR_HOST and will
 * use the emulator REST endpoint when present. You can also override the project
 * with FIREBASE_PROJECT and the API key with FIRESTORE_API_KEY (not needed for emulator).
 */
...
// Build base URL depending on whether we're talking to emulator or production
...
// Base TIE Fighter models that are NOT variants
const BASE_TIE_MODELS = ['tie-starfighter', 'tie-in-fighter'];

// List of TIE ship slugs to update
const TIE_SHIPS = [
