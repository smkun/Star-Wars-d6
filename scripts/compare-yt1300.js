#!/usr/bin/env node
/**
 * Compare YT-1300 data: d6holocron source vs our database
 */
...
async function compare() {
  ...
  const holocronData = await page.evaluate(() => {
    ...
    result.hyperdriveBackup = extractField('Hyperdrive Backup');
    ...
    // Extract weapons
    let weaponsMatch = text.match(/<b>Weapons:<\/b>\s*<\/p>\s*<ul>(.*?)<\/ul>/is);
    if (!weaponsMatch) {
      weaponsMatch = text.match(/<b>Weapons<\/b>:\s*<\/p>\s*<ul>(.*?)<\/ul>/is);
    }

    const weapons = [];
    if (weaponsMatch) {
      const weaponsList = weaponsMatch[1];
      const weaponItems = weaponsList.match(/<li>([^<]+)/g) || [];
      weaponItems.forEach(item => {
        const name = item.replace(/<li>/, '').trim();
  ...
  if (dbData.weapons_json) {
    dbData.weapons = JSON.parse(dbData.weapons_json);
    delete dbData.weapons_json;
  }
  ...
  // Check weapons
  const holocronWeapons = JSON.stringify(holocronData.weapons);
  const dbWeapons = JSON.stringify(dbData.weapons);
  if (holocronWeapons === dbWeapons) {
    console.log(`✅ weapons: MATCH`);
    matches++;
  } else {
    console.log(`❌ weapons: MISMATCH`);
    console.log(`   Holocron: ${holocronWeapons}`);
    console.log(`   Database: ${dbWeapons}`);
    mismatches++;
  }
