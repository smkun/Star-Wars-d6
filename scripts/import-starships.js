...
/**
 * Import starships to Firestore using REST API
 * Usage: node scripts/import-starships.js <import-ready-json>
 */
...
const importStarships = async (jsonPath) => {
  ...
  console.log(`Importing ${data.starships.length} starships...`);
  ...
  for (const starship of data.starships) {
    ...
    try {
      const response = await fetch(
        `https://firestore.googleapis.com/v1/projects/${FIREBASE_PROJECT}/databases/(default)/documents/starships?documentId=${slug}&key=${API_KEY}`,
        {
          method: 'POST',
          ...
          body: JSON.stringify({
            fields: {
              ...
              sensors: starship.sensors ? {
                mapValue: {
                  fields: {
                    passive: { stringValue: starship.sensors.passive || '' },
                  ...
                  values: (starship.weapons || []).map(weapon => ({
                    mapValue: {
                      fields: {
                        ...
                        atmosphereRange: { stringValue: weapon.atmosphereRange || '' },
...
const main = async () => {
  ...
  if (!jsonPath) {
    console.error('Usage: node scripts/import-starships.js <import-ready-json>');
    process.exit(1);
  }
  ...
  await importStarships(jsonPath);
};
