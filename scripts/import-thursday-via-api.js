/**
 * Import 6 new characters from Thursday.txt via API
 * Excludes: Long Claw and Bilar Saruun (already in DB)
 *
 * Usage: node scripts/import-thursday-via-api.js <firebase-token>
 * Get token from Firebase Console or use dev mode
 */
...
const characters = [
  {
    ...
    properties: {
      ...
      weapons: [
        'Hold Out Blaster (3D; 3–7/15/30; 6; +1D conceal in civvies)'
      ],
      armor: 'Chassis only',
      gear: [
  ...
  },
  {
    ...
    properties: {
      ...
      weapons: [
        'Sporting Blaster (3D+2; 3–12/30/120; 50; scoped)',
...
async function importViaAPI() {
  // Get Firebase token from command line or use dev mode
  const token = process.argv[2];

  if (!token) {
    console.error('❌ Usage: node scripts/import-thursday-via-api.js <firebase-token>');
    console.error('   Or use DEV_AUTH=true in API server for dev:scottkunian@gmail.com token');
    process.exit(1);
  }
  ...
  for (const char of characters) {
    try {
      // API expects: { name, species_slug, description, data: {...} }
      const payload = {
        name: char.name,
        species_slug: char.species_slug,
      ...
      if (!response.ok) {
        ...
        console.error(`❌ Failed to insert ${char.name}: HTTP ${response.status} - ${errorText}`);
        errors++;
        continue;
      }

      const result = await response.json();
      console.log(`✅ Inserted: ${char.name} (${result.id})`);
      inserted++;
    } catch (err) {
      console.error(`❌ Failed to insert ${char.name}:`, err.message);
      errors++;
    }
  }
